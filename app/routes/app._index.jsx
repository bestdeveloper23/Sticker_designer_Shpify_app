...
export const loader = async ({ request }) => {
  ...
  if (shop) {
    ...
    // App Store 1.2.2: require approval before use; redirect to confirmation when no active subscription.
    if (!billing.hasSubscription && billing.confirmationUrl) {
      return redirect(billing.confirmationUrl);
    }
    // Subscription create returned no confirmationUrl (e.g. API error); surface so merchant can retry.
    if (!billing.hasSubscription && !billing.confirmationUrl) {
      return {
        needsDraftOrderScope: !hasDraftOrderScope,
        billingSetupFailed: true,
      };
    }
  }
...
export default function Index() {
  ...
  return (
    <s-page heading="Shopify app template">
      ...
      {billingSetupFailed && (
        <s-banner tone="critical" className="mb-4">
          Billing could not be set up. Please refresh the page to try again, or go to Settings → Billing in your Shopify admin to approve the app charge.
        </s-banner>
      )}

      {needsDraftOrderScope && (
        <s-section heading="Draft orders (custom sticker size/quantity)">
          <s-paragraph>
            Grant permission to create draft orders so customers can order custom sizes and quantities from the sticker designer.
          </s-paragraph>
